...
If MainKeyHandle Then
   rtn = RegCreateKey(MainKeyHandle, Keyname, hKey)
   If rtn = ERROR_SUCCESS Then
...
If MainKeyHandle Then
   ...
   If rtn = ERROR_SUCCESS Then
      ...
      If rtn = ERROR_SUCCESS Then
        ...
        ClearKK sBuffer
        GetStringValue = Trim(Left(sBuffer, LenB(StrConv(sBuffer, vbFromUnicode)) - 1))
      Else
...
If MainKeyHandle Then
   rtn = RegCreateKey(MainKeyHandle, SubKey, hKey)
   If rtn = ERROR_SUCCESS Then
      rtn = RegSetValueEx(hKey, Entry, 0, REG_SZ, ByVal Value, LenB(StrConv(Value, vbFromUnicode)))
      If Not rtn = ERROR_SUCCESS Then
        If DisplayErrorMsg = True Then
            MsgBox ErrorMsg(rtn)
        End If
